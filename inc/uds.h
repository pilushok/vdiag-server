...
typedef enum {
    ...
    NRC_SUBFUNCTION_NOT_SUPPORTED_IN_SESSION = 0x7E,  // Wrong session for sub-function
 ...
 * @note Request SIDs are odd (0x11, 0x27, etc.), response SIDs are even (request SID + 0x40)
 */
typedef enum {
    ...
    UDS_SID_SECURITY_ACCESS_ADVANCED        = 0x29,  // PKI-based auth (table lists 0x28?)
    UDS_SID_COMMUNICATION_CONTROL           = 0x28,  // Enable/disable messaging

    /* --- Data Transmission Services --- */
    UDS_SID_ENABLE_DISABLE_ENCRYPTED_COMMS  = 0x84,  // ISO 15764 security (table lists 0x24?)
    UDS_SID_SET_TIMING_PARAMETERS           = 0x83,  // Adjust comms timing (table lists 0x30?)
...
typedef uds_error_t (*uds_tester_present_t)(uint8_t *preq, uint32_t ureq_len, uint8_t *presp, uint32_t *presp_len);
typedef uds_error_t (*uds_read_by_id_t)(uint8_t *preq, uint32_t ureq_len, uint8_t *presp, uint32_t *presp_len);
typedef uds_error_t (*uds_read_by_address_t)(uint8_t *preq, uint32_t ureq_len, uint8_t *presp, uint32_t *presp_len);
...
int uds_handle_msg(const uint8_t* request, size_t request_len, uint8_t* response,
                       size_t max_response_len, struct uds_state *puds);
